...
- `gemini_legacy`: implemented as keyed Gemini adapter for full Classic table generation. P024/P028 align the request with the source prototype `/api/translate-table` behavior: use `gemini-flash-latest`, send the prototype plain prompt through REST `generateContent`, do not set REST JSON response mode or extra generation config, parse the returned candidate text as JSON, and normalize `segments` + `rows` like source `buildRowsFromGeminiPayload`. Parser hardening accepts `{ "rows": [...] }` or top-level row arrays, repairs common malformed JSON such as missing commas between row objects before falling back to loose extraction, avoids regex in JSON repair to prevent Android `PatternSyntaxException`, and maps malformed Gemini HTTP/candidate envelopes to `InvalidResponse` instead of `Unknown`.
