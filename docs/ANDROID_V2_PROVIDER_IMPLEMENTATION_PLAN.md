...
- `gemini_legacy`: implemented as keyed Gemini adapter for full Classic table generation. P024 aligns the request with the source prototype `/api/translate-table` behavior: use `gemini-flash-latest`, send the prototype plain prompt through REST `generateContent`, do not set REST JSON response mode, parse the returned candidate text as JSON, and normalize `segments` + `rows` like source `buildRowsFromGeminiPayload`. Parser hardening accepts `{ "rows": [...] }` or top-level row arrays, recovers rows from partially malformed candidate JSON when row objects are still present, and maps malformed Gemini HTTP/candidate envelopes to `InvalidResponse` instead of `Unknown`.
