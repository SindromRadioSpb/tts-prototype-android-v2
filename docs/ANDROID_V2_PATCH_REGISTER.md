...
| P006 | M4 | In progress | Add TTS provider registry, fake TTS provider, Google Online TTS missing-configuration adapter, Android platform TTS fallback adapter, and deterministic audio asset keys. | `core/provider`, `data/provider/tts`, `TtsPrototypeApplication.kt`, TTS tests | `.\gradlew.bat test`; `.\gradlew.bat assembleDebug`; `.\gradlew.bat lint`; `git diff --check`; secret scan | Migration plan, provider plan/policy, audio architecture, QA strategy, traceability, risk register | Android platform TTS requires device/emulator smoke; Google Online TTS remains disabled until M9 secure settings | Pending | 2026-04-25 | Commit hash will be recorded by adjacent checkpoint docs commit after P006 lands. |
