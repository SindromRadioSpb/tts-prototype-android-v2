...
| P009 | M7 | In progress | Add Room-backed Library tab with browse, open, archive, restore, delete, row preview, and selected text lifecycle state. | `feature/library`, `ui/AppRoot.kt`, `MainActivity.kt`, `data/repository`, library tests | `.\gradlew.bat test`; `.\gradlew.bat assembleDebug`; `.\gradlew.bat lint`; `git diff --check`; secret scan | Migration plan, local library plan, Classic UI spec, QA strategy, traceability, UI evidence, risk register | Export UI remains disabled; row editing/reorder/reset UI remains M8 | Pending | 2026-04-25 | Commit hash will be recorded by adjacent checkpoint docs commit after P009 lands. |
