...
| P008 | M6 | In progress | Add audio-aware ZIP export writer with manifest, library JSON, missing audio report, safe path validation, and export history. | `core/export`, `data/export`, `LibraryDao.kt`, `TtsPrototypeApplication.kt`, export tests | `.\gradlew.bat test`; `.\gradlew.bat assembleDebug`; `.\gradlew.bat lint`; `git diff --check`; secret scan | Migration plan, export spec, QA strategy, traceability, UI evidence, risk register | SAF/share UI wiring remains future work; interrupted export needs UI evidence later | Pending | 2026-04-25 | Commit hash will be recorded by adjacent checkpoint docs commit after P008 lands. |
