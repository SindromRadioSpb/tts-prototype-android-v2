...
P021 fixes Gemini HTTP 400 handling: Android now uses the source-compatible `gemini-flash-latest` model alias, maps HTTP 400 by Google's error body instead of treating every 400 as an invalid API key, and clarifies that Settings validation is local format validation. P024 later supersedes the REST JSON-mode detail to match the source prototype's plain prompt contract.
...
P027 removes the remaining Android-only Gemini `generationConfig`, adds JSON repair before loose recovery, and preserves full row columns for common malformed Gemini output such as missing commas between row objects.
P028 removes regex from Gemini JSON repair itself. Missing commas between adjacent row objects and trailing commas before `]`/`}` are repaired with a JSON-aware scanner so Android runtime cannot fail with `PatternSyntaxException` on `}\s*\{`, and braces inside text cells remain untouched.
P029 wires Android Library v3 ZIP bundle import with audio: SAF `Импорт ZIP (с аудио)` opens a ZIP, imports `library/library.json`, stores bundled `audio/{sha256}.mp3` files into app-owned storage, links row/text audio in Room, and lets imported linked row audio play from local cache.
P030 keeps Classic Mode quick controls usable in landscape: bottom-right `✎ Редактор` remains visible across orientation, and bottom-left `↔ Альбом` / `↕ Портрет` requests portrait/landscape switching while respecting navigation and keyboard insets.
P031 specifies Library metadata context parity: a Library-loaded table must show saved `TITLE` and clickable/copyable `SOURCE` above the table, and Library cards must render the same `SOURCE` value in `Источник:` as clickable/copyable metadata.
P032 implements the P031 metadata source context: Classic state keeps the opened Library card `TITLE`/`SOURCE`, `Результат` shows them above the table, and the shared source row opens valid `http`/`https` links externally or copies the value.
P033 adds a fourth top-level `Связь` tab after IDE. WhatsApp is the primary support channel, Facebook/Instagram are secondary external links, and app support info can be copied without including user texts, libraries, audio, or credentials.
P034 renames the fourth tab visually to two-line `Связь` / `с разработчиком`, makes `Лимиты и квоты` collapsible, and separates generated rows into a dedicated collapsible `Таблица` block.
P035 specifies premium web-ZIP import provenance behavior: imported row audio remains local-cache-first, translation/TTS provenance is shown separately from active settings, and profile mismatches require explicit user action before re-synthesis.
...
| M11 - Import/compatibility layer | Medium | Import Android ZIP and compatible old web JSON where possible, preserving audio/profile/table provenance where source bundles expose it. | Import fixture tests and partial import tests. | Import remains read-only preview until safe. | P016 implements old web JSON import; P029 wires Android Library v3 ZIP bundle import with audio through SAF; P035 defines the remaining imported provenance/profile UX requirements. |
| M12 - Premium UI/UX polish | Medium | Implement and verify v4 screenshot-based Classic/Library mobile parity, accessibility, RTL, insets, long text, no clipped actions, orientation-safe quick controls, Library metadata context above loaded tables, and developer-contact UX. | UI DoD evidence, screenshot/manual smoke, accessibility smoke tests. | Keep functional UI if visual polish causes regressions. | P014 implements the first code pass; P030 adds landscape-safe Classic quick controls and an orientation toggle; P031 specifies saved `TITLE`/`SOURCE`; P032 implements code-level metadata/link behavior; P033 adds the support/contact tab; P034 separates `Результат` from `Таблица` and adds collapsible limits. Manual evidence and Compose UI tests remain required. |
